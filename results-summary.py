...
def summary_report(paths):
    ...
    for index,val in enumerate(row_labels):
        max_cons.append(df_dict[val]['Maximum consumption'].max())
        min_cons.append(df_dict[val]['Minimum consumption'].max())
        avg_total_cons.append(df_dict[val]['Total consumption'].mean())
        ...
        if station_visit_avg == 0:
            ...
            avg_reliability.append(round(station_visit_avg/station_refuel_avg)*100)

    pd.DataFrame({'Station': row_labels, 'Maximum consumption': max_cons,'Minimum consumption':max_cons,'Total consumption':avg_total_cons,'Trucks Visited':avg_truck_visit,'Trucks Refuelled':avg_truck_refuel,'Station Reliability':avg_reliability}).to_excel('Fuel-Station-Summary.xlsx',index=False)
